...
app.post('/api/sales', async (req, res) => {
	...
	try {
		...
		if(result1.recordset.length > 0) {
			...
			UPDATE_SEAT_STATUS(req.body.seat_no,1,CNST_STAFF_ID);
			REQUEST_LOG(req,`WARN: ${CNST_ERROR_CODE.error_2} SEAT_NO Seat number not found`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_2);
		}
	...
	function UPDATE_SEAT_STATUS(seatNo,seatStatus,staffId) {
		let SQL = '';
		let result;
		try {
			...
			let UPDATE_MST_SEAT = pool.request()
...
app.post('/api/paid', async (req,res) => {
	...
	async function SINGLE_SALES_NO(uriageDtlObj,PRICE_LIMIT_FLG) {
		...
		try {
			for(let iSalesData in uriageDtlObj.SALES_DATA) {
				...
				let _UPDATE_TBL_GATE = await UPDATE_TBL_GATE(JSON_TBL_URIAGE,50000);
	...
	async function MULTIPLE_SALES_NO(salesData,GassanSeq,PRICE_LIMIT_FLG) { // Gassan Log-out
		...
		try {
			...
			for(let iSalesData in salesData.SALES_DATA) {
				...
				let _UPDATE_TBL_GATE = await UPDATE_TBL_GATE(JSON_TBL_URIAGE,50000,iSalesData);
			...
			// INSERT TBL_CREDIT_RIREKI
			let { TBL_CREDIT_RIREKI } = salesData;
			let { SALES_NO } = TBL_CREDIT_RIREKI;
			if(SALES_NO != null) {
	...
	async function UPDATE_TBL_GATE(uriage,poolPrice,SEQ = 0) {
		let SQL = "";
		const SEISAN_DATE = dateTimeNow();
		try{
			...
			let flg = () => {
				return {
					OPEN_FLG: (uriage.URIAGE_YEN >= poolPrice)?1:0,
					PRICE_LIMIT_FLG: (uriage.URIAGE_YEN >= poolPrice)?1:0
				};
			};
...
app.post('/testrollback', async (req,res) => {
	...
	const hash = crypto.createHash('md5').update('dsa').digest("hex");
	console.log(hash);
