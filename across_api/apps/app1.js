...
let userPassValidation = function(req,res,next) {
	...
	try{
		...
		console.log(err);
		res.send(CNST_ERROR_CODE.error_3);
		return;
	}
};
...
app.post('/api/sales', async (req, res) => {
	...
	async function FILTER_URIAGE_DTL(arrObj) {
		for(let i in arrObj) {
			let iObj = arrObj[i];
			tempUriageDtl[iObj.SALES_NO] = iObj;
		}
	}
...
app.post('/api/paid', async (req,res) => {
	...
	// GET TOKENIZED FROM TBL_URIAGE_DTL_TEMP
	...
	if(_VALIDATE_LOGOUT_PARAM) {
		...
		if(salesCount == 1) {
			...
			try {
				...
				// uriage_yen = Shop.TAX_FLG == Var.TAX_FLG_INCLUDE ? Convert.ToInt32(lblZanSeisanYen.Value + lblMaebaraiYen.Value + lblCreditYen.Value) : Convert.ToInt32(lblZanSeisanYen.Value + lblMaebaraiYen.Value + lblCreditYen.Value);
				// uriage_yen = uriage_yen < 0 ? 0 : uriage_yen;
				...
				.input("GASSAN_CNT", sql.Int, salesCount)
				...
				.query(SQL);
				res.send(CNST_ERROR_CODE.error_0);
			} catch(err) {
	...
	async function SINGLE_SALES_NO(uriageDtlObj) {
		...
		try {
			...
			ERROR_LOGGER(0,'SINGLE_SALES_NO: '+err);
		}
		closeConnection();
	}
	...
	async function MULTIPLE_SALES_NO(salesData,GassanSeq,newMemberCnt) { // Gassan Log-out
	...
	async function UPDATE_SEAT_STATUS(seatNo,seatStatus,staffId) {
		...
		try {
			...
			ERROR_LOGGER(0,'UPDATE_SEAT_STATUS: '+err);
		}
	...
	async function TBL_URIAGE(SEAT_NO,SALES_NO) {
		try{
			...
			if(result.recordset.length > 0) {
				...
				ERROR_LOGGER(0,'No match found');
			}
		} catch(err) {
			ERROR_LOGGER(0,'TBL_URIAGE: '+err);
		}
	...
	async function SEAT_ITEM_USE_MIN(salesNo,seisanDate) {
		...
		try{
			...
			result = await pool.request()
			.input('SALES_NO', sql.VarChar(4), salesNo)
			.input('END_DATE', sql.VarChar(50), seisanDate)
			.input('DELETE_FLG', sql.TinyInt, 0)
			.input('SEISAN_FLG', sql.TinyInt, 0)
			.query("SELECT OCCUPIED_TIME = DATEDIFF(MINUTE, LOGIN_DATE, '2018-05-09 08:53:50') FROM TBL_SEAT_STATUS WHERE SALES_NO = '000000042100' AND DELETE_FLG = 0 AND SEISAN_FLG = 0;");
