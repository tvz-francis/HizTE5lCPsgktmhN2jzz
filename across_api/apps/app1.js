...
const config = {
	...
	// database:'POS-_-99997-_-07_25'
	// database:'POS-_-00264-_-08_01'
};
...
app.post('/api/sales', async (req, res) => {
	...
	for(let i in rules) {
		if(!req.body[i]) {
			...
			MONITOR_LOG(200,logMsg,data,res);
		}
	}
	...
	try {
		...
		if(result1.recordset.length >> 1) {
			...
			for(let iSeatData in result1.recordset) {
				...
				/* SHOWER VALIDATION */
				SQL = "SELECT * FROM TBL_SHOWER_USE WHERE USE_FLG = 1 AND SALES_NO = @SALES_NO";
				let checkShower = await pool.request()
				.input('SALES_NO', sql.VarChar(12), SALES_NO)
				.query(SQL);
				if(checkShower.recordset.length > 0) {
					// let _UPDATE_SEAT_STATUS_ = await UPDATE_SEAT_STATUS(SEAT_NO,1,CNST_STAFF_ID);
					// REQUEST_LOG(req,`WARN: ${CNST_ERROR_CODE.error_8} Shower Validation`);
					// sql.close();
					// return res.status(200).send(CNST_ERROR_CODE.error_8);
					salesDataCnt-1;
					result1.recordset.splice(iSeatData,1,'undefined');
				}

				/* RENTAL VALIDATION */
				SQL = "SELECT ISNULL(SUM(ITEM_QU-RETURN_QU),0) AS TOTAL_QU FROM TBL_URIAGE_DTL WHERE SALES_NO=@SALES_NO AND ITEM_KBN=@ITEM_KBN AND SEISAN_FLG=@SEISAN_FLG AND DELETE_FLG=@DELETE_FLG";
				let checkRental = await pool.request()
				.input("SALES_NO", sql.VarChar(12), SALES_NO)
				.input("ITEM_KBN", sql.Int, 4)
				...
				SQL = "SELECT * FROM TBL_AWAY_SHOP WHERE SALES_NO = @SALES_NO AND MEMBER_ID = @MEMBER_ID AND BACK_TIME IS NULL";
		...
		} else if(result1.recordset.length === 1) {
			...
			for(let iSeatData in result1.recordset) {
				...
				if(checkShower.recordset.length > 0) {
					...
					//let _UPDATE_SEAT_STATUS_ = await UPDATE_SEAT_STATUS(SEAT_NO,SEAT_STATUS,CNST_STAFF_ID);
				...
				if(checkRental.recordset[0].TOTAL_QU != 0) {
					//Edit By Gary
					//UPDATE_SEAT_STATUS(SEAT_NO,1,CNST_STAFF_ID);
					let _UPDATE_SEAT_STATUS_ = await UPDATE_SEAT_STATUS(SEAT_NO,1,CNST_STAFF_ID); 
					// UPDATE SEAT_STATUS
					// let SQL = "UPDATE MST_SEAT SET SEAT_STATUS = @SEAT_STATUS, UPDATE_DATE = GETDATE(), UPDATE_STAFF_ID = @UPDATE_STAFF_ID WHERE SEAT_NO = @SEAT_NO";
					// let UPDATE_MST_SEAT = await pool.request()
			...
			let _UPDATE_SEAT_STATUS_ = await UPDATE_SEAT_STATUS(req.body.seat_no,1,CNST_STAFF_ID);
	...
	async function UPDATE_SEAT_STATUS(seatNo,seatStatus,staffId) {
		...
		try {
			...
			REQUEST_LOG(req,`ERROR: ${ERROR_LOGGER.error_11} UPDATE_SEAT_STATUS ${err}`);
			sql.close();
			return res.status(200).send(ERROR_LOGGER.error_11);
		}
				...
				if(obj.ITEM_KBN !== 0) {
					...
					for(let i2 in query.recordset) {
						...
						if(obj2.AUTO_PACK_ID != null) {
							let autoPack = await AUTO_PACK(obj2.ITEM_ID,obj2.SEAT_USE_START_DATE,SEISAN_DATE,_uriage.MEMBER_FLG,obj2.ITEM_SEQ,convert_datetime(obj2.SEAT_USE_START_DATE));
								_tbl_uriage_class.push({
	...
	async function AUTO_PACK(itemId,seatUseStartDate,seatUseEndDate,uriageMemberFlg,itemSeq,loginDate,useCount = 1) {
		...
		try {
			...
			let getEndDate = loginDate.split(' ')[1];
			...
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} AUTO_PACK ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
	...
	async function CALC_EXPRICE_VIRTUAL(seatUseStartDate,seatUseEndDate,memberFlg,itemBaseMin,exItemId,exBaseMin,packEndTime,useCount) {
		...
		try {
			...
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} CALC_EXPRICE_VIRTUAL ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
	...
	async function CALC_TOTAL_MINS(seatUseStartDate,seatUseEndDate) {
		...
		try {
			...
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} CALC_TOTAL_MINS ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
	...
	async function GET_SEAT_ITEM_END_DATE(data) {
		try	{
			...
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} GET_SEAT_ITEM_END_DATE ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
	...
	async function TAX_RATE(date) {
		...
		try{
			SQL = "";
			let query = await pool.request()
			.input('TAXDATE',sql.Date,date)
			.query("SELECT TOP 1 ISNULL(TAX_RATE,0) AS TAX_RATE FROM MST_TAX WHERE START_DATE < CONVERT(char(8), @TAXDATE ,112)");
			...
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} TAX_RATE ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
	...
	async function TBL_URIAGE_DTL(SALES_NO) {
		...
		try {
			...
			.query("SELECT * FROM [TBL_URIAGE_DTL] AS [TBL_URIAGE_DTL] WHERE [TBL_URIAGE_DTL].[SALES_NO] = @salesno AND SEISAN_FLG = 0 AND DELETE_FLG = 0 ORDER BY SEQ ASC");
			return result2.recordset;
		} catch(err) {
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} TBL_URIAGE_DTL ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
	...
	async function SEAT_ITEM_USE_MIN(salesNo,seisanDate) {
		...
		try{
			...
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} SEAT_ITEM_USE_MIN ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
	...
	async function COMPUTE_TOTAL_YEN(temp_uriage_dtl,tbluriage) {
		...
		try {
			...
			// REQUEST_LOG(req,`Error: ${err}`);
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} COMPUTE_TOTAL_YEN ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
	...
	async function GET_MST_SHOP() {
		try {
			...
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} GET_MST_SHOP ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
	...
	async function CALC_TAX_PRICE(taxRate,price,taxKbn) {
		...
		try {
			...
			if(taxKbn == 0) {
				...
				result = Math.floor(price * (taxRate / 100));
			}

		} catch(err) {
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} CALC_TAX_PRICE ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
...
app.post('/api/gassan_sales_no', async (req,res) => {
	...
	let GASSAN_SALES_NO = 0;

	try {

		let MST_SHOP_query = "SELECT NEXT VALUE FOR ARG_GASSAN_SALES_NO AS ARG_GASSAN_SALES_NO";
		...
		if(MST_SHOP_query.recordset.length > 0) {
			...
			GASSAN_SALES_NO = parseInt(MST_SHOP_query.ARG_GASSAN_SALES_NO);

			// update mst shop
			let UPDATE_MST_SHOP = await pool.request()
			.input('GASSAN_SALES_NO', sql.BigInt, GASSAN_SALES_NO)
			.query("UPDATE [MST_SHOP] SET [ARG_GASSAN_SALES_NO] = @GASSAN_SALES_NO")
			.then((result) => {
				if(result.rowsAffected > 0) {
					return_json.GASSAN_SALES_NO = padding(GASSAN_SALES_NO,11,'0');
					...
					REQUEST_LOG(req,`WARN: ${CNST_ERROR_CODE.error_11} MST_SHOP did not update`);
					sql.close();
					return res.status(200).send(return_json = CNST_ERROR_CODE.error_11);
				}
			});
			
		} else {
			REQUEST_LOG(req,`WARN: ${CNST_ERROR_CODE.error_11} MST_SHOP no record found`);
			sql.close();
			return res.status(200).send(return_json = CNST_ERROR_CODE.error_11);
		}
	...
	function padding(target,num,padded) {
		try{
			...
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} padding ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
...
app.post('/api/init', async (req,res) => {
	let return_json = {};
	const BASE_MIN = 120;
	// const EX_BASE_MIN = 50;
	let pool = await sql.connect(config);
	try {

		let EX_BASE_MIN_SQL = "SELECT EX.BASE_MIN FROM MST_SEAT_ITEM S INNER JOIN MST_EX_SEAT_ITEM EX ON EX.ITEM_ID = S.EX_ITEM_ID WHERE S.ITEM_ID = '999920176262' AND S.SEQ = 0 AND EX.SEQ = 0";
		let EX_BASE_MIN = await pool.request()
		.query(EX_BASE_MIN_SQL);
		...
		if(MST_SHOP.recordset.length == 0)  {
			REQUEST_LOG(req,`WARN: ${CNST_ERROR_CODE.error_2} MST_SHOP no record found`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_2);
		}
		...
		if(MST_STAFF.recordset.length == 0)  {
			REQUEST_LOG(req,`WARN: ${CNST_ERROR_CODE.error_2} MST_STAFF no record found`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_2);
		}
		...
		if(MST_TAX.recordset.length == 0)  {
			REQUEST_LOG(req,`WARN: ${CNST_ERROR_CODE.error_2} MST_TAX no record found`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_2);
		}
		...
		let mstShopDetails = {
			...
			"MST_EX_SEAT_ITEM":{
				"BASE_MIN" : BASE_MIN,
				"EX_BASE_MIN": EX_BASE_MIN.recordset[0].BASE_MIN
			},
			"MST_STAFF" : {
		...
		REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} API-INIT ${err}`);
		sql.close();
		return res.status(200).send(CNST_ERROR_CODE.error_11);
	}
...
app.post('/api/deposit', async (req,res) => {
	...
	try {
		...
		.then(async(result) => {
			...
			if(VALIDATE_SALES_NO.recordset.length > 0) {
				...
				if(!regexp.test(DEPOSIT_AMOUNT)) {
					REQUEST_LOG(req,`WARN: ${CNST_ERROR_CODE.error_2} VALIDATE DEPOSIT VALUE`);
					sql.close();
					return res.status(200).send(CNST_ERROR_CODE.error_2);
				}
				...
				if(GET_TBL_URIAGE.recordset.length !== 0 && TBL_GATE_SEQ.recordset.length !== 0) {
					...
					REQUEST_LOG(req,`WARN: ${CNST_ERROR_CODE.error_2} TBL_URIAGE and TBL_GATE no data found`);
					sql.close();
					return res.status(200).send(CNST_ERROR_CODE.error_2);
				}

			} else {
				REQUEST_LOG(req,`WARN: ${CNST_ERROR_CODE.error_2} TBL_URIAGE no data`);
				sql.close();
				return res.status(200).send(CNST_ERROR_CODE.error_2);
			}
		...
		.catch((err) => {
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_3} API-DEPOSIT post validation`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_3);
		});

	} catch(err) {
		REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} API-DEPOSIT ${err}`);
		sql.close();
		return res.status(200).send(CNST_ERROR_CODE.error_11);
	}
...
app.post('/api/paid', async (req,res) => {
	...
	if(_VALIDATE_LOGOUT_PARAM) {
		...
		if(salesCount == 1) {
			...
			let DO_SINGLE_SALES_NO = await SINGLE_SALES_NO(reqParam, parseInt(reqParam.ALL_TOTAL), PRICE_LIMIT_FLG);
			// SQL = "UPDATE MST_SEAT SET SEAT_STATUS = @SEAT_STATUS, UPDATE_DATE = GETDATE(), UPDATE_STAFF_ID = @UPDATE_STAFF_ID WHERE SEAT_NO = @SEAT_NO";
			// let UPDATE_MST_SEAT = await pool.request()
			...
			let { SALES_DATA,TBL_GASSAN,TBL_CREDIT_RIREKI } = reqParam;
			let GassanSeq = 0;
			let SQL = '';
			//#region TBL_GASSAN
			try {
				let VisitorCount = await MULTIPLE_SALES_NO(reqParam,GassanSeq, parseInt(reqParam.ALL_TOTAL), PRICE_LIMIT_FLG);
				...
				TBL_GASSAN["NEW_MEMBER_CNT"] = newMemberCnt;
				TBL_GASSAN["INPUT_DATE"] = SEISAN_DATE;
				TBL_GASSAN["UPDATE_DATE"] = dateTimeNow();

				let UDS = await UPLOAD_DATA_SALES(SALES_DATA,TBL_CREDIT_RIREKI,TBL_GASSAN);
				//close connection
				
				let _END_PROC = await END_PROC();
			} catch(err) {
				// REQUEST_LOG(req,`Error: AFTER MULTIPLE_SALES_DATA ${err}`);
				REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} ${err}`);
				sql.close();
				return res.status(200).send(CNST_ERROR_CODE.error_11);
		...
		REQUEST_LOG(req,`WARN: ${CNST_ERROR_CODE.error_11} VALIDATE_LOGOUT_PARAM`);
		sql.close();
		return res.status(200).send(CNST_ERROR_CODE.error_11);
	...
	function UPLOAD_SALES(jsonSales) {
		...
		const request_opt = {
			...
			body: jsonSales,
			json: true,
			url: 'http://api.across-web.net/upload/sales'
		...
		try {
			...
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} UPLOAD_SALES ${err}`);
			...
			for(let i = 0; i < uriageDtl.length; i++) {
				...
				if(query.recordset.length > 0) {
					for(let iUriageDtl = 0; iUriageDtl < query.recordset.length;iUriageDtl++) {
						...
						query.recordset[iUriageDtl].SEISAN_DATE = (query.recordset[iUriageDtl].SEISAN_DATE == null)?null:convert_datetime(query.recordset[iUriageDtl].SEISAN_DATE);
	...
	async function TBL_SEAT_STATUS(tblSeatStatus,loginHolidayFlg,seisanHolidayFlg,shopFcNo) {
		...
		try {
			...
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} TBL_SEAT_STATUS ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
	...
	async function VALIDATE_LOGOUT_PARAM(data) {
		...
		try {
			...
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} VALIDATE_LOGOUT_PARAM ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
		}
		return bool;
	...
	async function SINGLE_SALES_NO(uriageDtlObj, allTotalSaleYen, PRICE_LIMIT_FLG) {
		...
		try {
			for(let iSalesData in uriageDtlObj.SALES_DATA) {
				...
				.input('SEISAN_DATE', sql.VarChar, JSON_TBL_URIAGE.SEISAN_DATE)
				...
				.input('MAEUKE_YEN', sql.Int, JSON_TBL_URIAGE.MAEUKE_YEN)
				...
				for(let i in JSON_TBL_URIAGE_DTL) {
					...
					jsonUriageDtl.ITEM_PRICE = jsonUriageDtl.ITEM_PRICE.toString();
					jsonUriageDtl.TOTAL_YEN = jsonUriageDtl.TOTAL_YEN.toString();
					...
					if(jsonUriageDtl.ITEM_KBN == 0) {
						...
						.input('BASE_MIN', sql.Int, jsonUriageDtl.BASE_MIN)
						.input('ITEM_KBN', sql.Int, jsonUriageDtl.ITEM_KBN)
						.input('SEISAN_DATE', sql.VarChar, JSON_TBL_URIAGE.SEISAN_DATE)
					...
					if(UPDATE_TBL_URIAGE_DTL.rowsAffected[0] == 0) {
						...
						.input("ITEM_NM", sql.NVarChar(120), jsonUriageDtl.ITEM_NM)
						.input("ITEM_KBN", sql.Int, jsonUriageDtl.ITEM_KBN)
						.input("FOOD_KBN", sql.Int, jsonUriageDtl.FOOD_KBN)
						.input("TAX_KBN", sql.Int, taxFlg)
						.input("ITEM_QU", sql.Int, jsonUriageDtl.ITEM_QU)
						.input("ITEM_PRICE", sql.VarChar, jsonUriageDtl.ITEM_PRICE)
						.input("TOTAL_YEN", sql.VarChar, jsonUriageDtl.TOTAL_YEN)
						.input("SEISAN_DATE", sql.VarChar, JSON_TBL_URIAGE.SEISAN_DATE)
						.input("SEISAN_FLG", sql.VarChar, 1)
				...
				//Edit by Gary
				let _UPDATE_TBL_GATE = await UPDATE_TBL_GATE(JSON_TBL_URIAGE, allTotalSaleYen, 50000);
				
				// MST_SEAT SEAT_STATUS
				//let _UPDATE_SEAT_STATUS_ = await UPDATE_SEAT_STATUS(JSON_TBL_URIAGE.SEAT_NO,JSON_MST_SEAT.SEAT_STATUS,CNST_STAFF_ID);
			}

			// INSERT TBL_CREDIT_RIREKI
			let { TBL_CREDIT_RIREKI } = uriageDtlObj;
			let { SALES_NO } = TBL_CREDIT_RIREKI;
			if(SALES_NO != null) {
			
				if(TBL_CREDIT_RIREKI) {

					/* GET SEQ */
					let CREDIT_RIREKI_SEQ = await pool.request()
					.input('SALES_NO', sql.VarChar, TBL_CREDIT_RIREKI.SALES_NO)
					.query("SELECT MAX(SEQ) AS SEQ FROM TBL_CREDIT_RIREKI WHERE SALES_NO = @SALES_NO");
					let CREDIT_SEQ = 0;
					if(CREDIT_RIREKI_SEQ.recordset[0].SEQ !== null) {
						let INSERT_CREDIT_RIREKI = await pool.request()
						.input('SALES_NO', sql.VarChar, TBL_CREDIT_RIREKI.SALES_NO)
						.input('GASSAN_SALES_NO', sql.VarChar, TBL_CREDIT_RIREKI.GASSAN_SALES_NO)
						.input('SEQ', sql.VarChar, CREDIT_RIREKI_SEQ.recordset[0].SEQ)
						.input('MEMBER_ID', sql.VarChar, TBL_CREDIT_RIREKI.MEMBER_ID)
						.input('INPUT_PRICE', sql.VarChar, TBL_CREDIT_RIREKI.INPUT_PRICE)
						.input('CREDIT_FLG', sql.VarChar, TBL_CREDIT_RIREKI.CREDIT_FLG)
						.input('PAYBACK_FLG', sql.VarChar, TBL_CREDIT_RIREKI.PAYBACK_FLG)
						.input('DELETE_FLG', sql.VarChar, TBL_CREDIT_RIREKI.DELETE_FLG)
						.input('SEISAN_FLG', sql.VarChar, TBL_CREDIT_RIREKI.SEISAN_FLG)
						.input('SEISAN_DATE', sql.VarChar, TBL_CREDIT_RIREKI.SEISAN_DATE)
						.input('INPUT_STAFF_ID', sql.VarChar, TBL_CREDIT_RIREKI.INPUT_STAFF_ID)
						.input('INPUT_DATE', sql.VarChar, TBL_CREDIT_RIREKI.INPUT_DATE)
						.query("INSERT INTO TBL_CREDIT_RIREKI(SALES_NO,SEQ,MEMBER_ID,INPUT_PRICE,CREDIT_FLG,PAYBACK_FLG,DELETE_FLG,SEISAN_FLG,SEISAN_DATE,INPUT_STAFF_ID,INPUT_DATE) VALUES(@SALES_NO,@SEQ,@MEMBER_ID,@INPUT_PRICE,@CREDIT_FLG,@PAYBACK_FLG,@DELETE_FLG,@SEISAN_FLG,@SEISAN_DATE,@INPUT_STAFF_ID,@INPUT_DATE)");
					} else {
						let INSERT_CREDIT_RIREKI = await pool.request()
						.input('SALES_NO', sql.VarChar, TBL_CREDIT_RIREKI.SALES_NO)
						.input('GASSAN_SALES_NO', sql.VarChar, TBL_CREDIT_RIREKI.GASSAN_SALES_NO)
						.input('SEQ', sql.VarChar, CREDIT_SEQ)
						.input('MEMBER_ID', sql.VarChar, TBL_CREDIT_RIREKI.MEMBER_ID)
						.input('INPUT_PRICE', sql.VarChar, TBL_CREDIT_RIREKI.INPUT_PRICE)
						.input('CREDIT_FLG', sql.VarChar, TBL_CREDIT_RIREKI.CREDIT_FLG)
						.input('PAYBACK_FLG', sql.VarChar, TBL_CREDIT_RIREKI.PAYBACK_FLG)
			...
			let { SEAT_STATUS } =  reqParam.SALES_DATA[0].MST_SEAT;
			...
			//Edit By Gary
			if(PRICE_LIMIT_FLG){
				_UPDATE_SEAT_STATUS = await UPDATE_SEAT_STATUS(SEAT_NO,11,CNST_STAFF_ID);
			}else{
				_UPDATE_SEAT_STATUS = await UPDATE_SEAT_STATUS(SEAT_NO,SEAT_STATUS,CNST_STAFF_ID);
			}

			// _UPDATE_SEAT_STATUS = await UPDATE_SEAT_STATUS(SEAT_NO,seatStatus,CNST_STAFF_ID);
			let UDS = await UPLOAD_DATA_SALES(uriageDtlObj.SALES_DATA,uriageDtlObj.TBL_CREDIT_RIREKI);
		} catch (err) {
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} SINGLE_SALES_NO ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
		}
	...
	async function MULTIPLE_SALES_NO(salesData,GassanSeq, allTotalSaleYen, PRICE_LIMIT_FLG) { // Gassan Log-out
		...
		try {
			...
			for(let iSalesData in salesData.SALES_DATA) {
				...
				// // INSERT TBL_CREDIT_RIREKI
				// if(JSON_TBL_CREDIT_RIREKI) {

				// 	/* GET SEQ */
				// 	let CREDIT_RIREKI_SEQ = await pool.request()
				// 	.input('SALES_NO', sql.VarChar, JSON_TBL_CREDIT_RIREKI.SALES_NO)
				// 	.query("SELECT MAX(SEQ) AS SEQ FROM TBL_CREDIT_RIREKI WHERE SALES_NO = @SALES_NO");
				// 	let CREDIT_SEQ = 0;
				// 	if(CREDIT_RIREKI_SEQ.recordset[0].SEQ !== null) {
				// 		let INSERT_CREDIT_RIREKI = await pool.request()
				// 		.input('SALES_NO', sql.VarChar, JSON_TBL_CREDIT_RIREKI.SALES_NO)
				// 		.input('SEQ', sql.VarChar, CREDIT_RIREKI_SEQ.recordset[0].SEQ)
				// 		.input('MEMBER_ID', sql.VarChar, JSON_TBL_CREDIT_RIREKI.MEMBER_ID)
				// 		.input('INPUT_PRICE', sql.VarChar, JSON_TBL_CREDIT_RIREKI.INPUT_PRICE)
				// 		.input('CREDIT_FLG', sql.VarChar, JSON_TBL_CREDIT_RIREKI.CREDIT_FLG)
				// 		.input('PAYBACK_FLG', sql.VarChar, JSON_TBL_CREDIT_RIREKI.PAYBACK_FLG)
				// 		.input('DELETE_FLG', sql.VarChar, JSON_TBL_CREDIT_RIREKI.DELETE_FLG)
				// 		.input('SEISAN_FLG', sql.VarChar, JSON_TBL_CREDIT_RIREKI.SEISAN_FLG)
				// 		.input('SEISAN_DATE', sql.VarChar, JSON_TBL_CREDIT_RIREKI.SEISAN_DATE)
				// 		.input('INPUT_STAFF_ID', sql.VarChar, JSON_TBL_CREDIT_RIREKI.INPUT_STAFF_ID)
				// 		.input('INPUT_DATE', sql.VarChar, JSON_TBL_CREDIT_RIREKI.INPUT_DATE)
				// 		.query("INSERT INTO TBL_CREDIT_RIREKI(SALES_NO,SEQ,MEMBER_ID,INPUT_PRICE,CREDIT_FLG,PAYBACK_FLG,DELETE_FLG,SEISAN_FLG,SEISAN_DATE,INPUT_STAFF_ID,INPUT_DATE) VALUES(@SALES_NO,@SEQ,@MEMBER_ID,@INPUT_PRICE,@CREDIT_FLG,@PAYBACK_FLG,@DELETE_FLG,@SEISAN_FLG,@SEISAN_DATE,@INPUT_STAFF_ID,@INPUT_DATE)");
				// 	} else {
				// 		let INSERT_CREDIT_RIREKI = await pool.request()
				// 		.input('SALES_NO', sql.VarChar, JSON_TBL_CREDIT_RIREKI.SALES_NO)
				...
				//Edit by Gary
				let _UPDATE_TBL_GATE = await UPDATE_TBL_GATE(JSON_TBL_URIAGE, allTotalSaleYen, 50000, iSalesData);

				// UDPATE MST_SEAT SEAT_STATUS
				//let _UPDATE_SEAT_STATUS_ = await UPDATE_SEAT_STATUS(JSON_TBL_URIAGE.SEAT_NO,JSON_MST_SEAT.SEAT_STATUS,CNST_STAFF_ID);
				// _UPDATE_SEAT_STATUS = await UPDATE_SEAT_STATUS(JSON_TBL_URIAGE.SEAT_NO,JSON_MST_SEAT.SEAT_STATUS,CNST_STAFF_ID);
			}

			// INSERT TBL_CREDIT_RIREKI
			let { TBL_CREDIT_RIREKI,TBL_GASSAN } = salesData;
			let { SALES_NO } = TBL_CREDIT_RIREKI;
			if(SALES_NO != null) {
				...
				if(TBL_CREDIT_RIREKI) {
					...
					if(CREDIT_RIREKI_SEQ.recordset[0].SEQ !== null) {
						...
						.input('INPUT_STAFF_ID', sql.VarChar, TBL_CREDIT_RIREKI.INPUT_STAFF_ID)
						.input('INPUT_DATE', sql.VarChar, TBL_CREDIT_RIREKI.INPUT_DATE)
						.query("INSERT INTO TBL_CREDIT_RIREKI(SALES_NO,GASSAN_SALES_NO,SEQ,MEMBER_ID,INPUT_PRICE,CREDIT_FLG,PAYBACK_FLG,DELETE_FLG,SEISAN_FLG,SEISAN_DATE,INPUT_STAFF_ID,INPUT_DATE) VALUES(@SALES_NO,@GASSAN_SALES_NO,@SEQ,@MEMBER_ID,@INPUT_PRICE,@CREDIT_FLG,@PAYBACK_FLG,@DELETE_FLG,@SEISAN_FLG,@SEISAN_DATE,@INPUT_STAFF_ID,@INPUT_DATE)");
					} else {
						let INSERT_CREDIT_RIREKI = await pool.request()
						.input('SALES_NO', sql.VarChar, TBL_CREDIT_RIREKI.SALES_NO)
						.input('GASSAN_SALES_NO', sql.VarChar, TBL_CREDIT_RIREKI.GASSAN_SALES_NO)
			...
			// let UDS = await UPLOAD_DATA_SALES(salesData.SALES_DATA,TBL_CREDIT_RIREKI,TBL_GASSAN);
		} catch(err) {
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} MULTIPLE_SALES_NO ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
	...
	async function UPLOAD_DATA_SALES(SALES_DATA,TBL_CREDIT_RIREKI,TBL_GASSAN = null) {
		...
		try {
			...
			let GASSAN_CNT = SALES_DATA.length;
			...
			for(let iSalesData in SALES_DATA) {
				...
				TBL_CREDIT_RIREKI["SEISAN_HOLIDAY_FLG"] = SEISAN_HOLIDAY_FLG;
				TBL_CREDIT_RIREKI["SHOP_FC_NO"] = _MST_SHOP.SHOP_FC_NO;

				let gassan_query_1 = await pool.request()
				.input('SALES_NO', sql.VarChar, JSON_TBL_URIAGE.SALES_NO)
				.query("SELECT TOP 1 GASSAN_SALES_NO FROM TBL_URIAGE WHERE SALES_NO = @SALES_NO");
				gassan_query_1 = gassan_query_1.recordset[0];

				let GASSAN_TBL = await pool.request()
				.input('GASSAN_NO', sql.VarChar, gassan_query_1.GASSAN_SALES_NO)
				.query("SELECT * FROM TBL_GASSAN WHERE GASSAN_NO = @GASSAN_NO");
				GASSAN_TBL = GASSAN_TBL.recordset[0];

				if(TBL_GASSAN != null) {
					TBL_GASSAN["SHOP_FC_NO"] = _MST_SHOP.SHOP_FC_NO;
					TBL_GASSAN["GASSAN_NO"] = TBL_GASSAN.GASSAN_NO;
					TBL_GASSAN["GASSAN_CNT"] = GASSAN_TBL.GASSAN_CNT;
					TBL_GASSAN["TOTAL_YEN"] = TBL_GASSAN.TOTAL_YEN;
					TBL_GASSAN["AZUKARI_YEN"] = TBL_GASSAN.AZUKARI_YEN;
					TBL_GASSAN["CREDIT_YEN"] = TBL_GASSAN.CREDIT_YEN;
					TBL_GASSAN["CHANGE_YEN"] = TBL_GASSAN.CHANGE_YEN;
					// TBL_GASSAN["INPUT_DATE"] = GASSAN_TBL.INPUT_DATE;
					TBL_GASSAN["INPUT_HOLIDAY_FLG"] = SEISAN_HOLIDAY_FLG;
					TBL_GASSAN["INPUT_STAFF_ID"] = GASSAN_TBL.INPUT_STAFF_ID;
					TBL_GASSAN["DELETE_FLG"] = GASSAN_TBL.DELETE_FLG;
					TBL_GASSAN["UPDATE_STAFF_ID"] = GASSAN_TBL.UPDATE_STAFF_ID;
					// TBL_GASSAN["UPDATE_DATE"] = GASSAN_TBL.UPDATE_DATE;
					TBL_GASSAN["TAX_YEN"] = TBL_GASSAN.TAX_YEN;
					TBL_GASSAN["TAX_KBN"] = GASSAN_TBL.TAX_KBN;
					TBL_GASSAN["MAEBARAI_YEN"] = TBL_GASSAN.MAEBARAI_YEN;
					TBL_GASSAN["URIAGE_YEN"] = TBL_GASSAN.URIAGE_YEN;
					TBL_GASSAN["NEW_MEMBER_CNT"] = GASSAN_TBL.NEW_MEMBER_CNT;
				}
			...
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} UPLOAD_DATA_SALES ${err}`);
		}
	...
	async function ICPOS_XML_MEMBER(memberID,shopFcNo,shopName) {
		...
		try {
			...
			SQL = `SELECT
			@SHOP_FC_NO AS SHOP_FC_NO,
			@SHOP_NAME AS SHOP_NM,
			M.MEMBER_ID,
			M.MEMBER_NM,
			M.MEMBER_SEI_KANA+' '+M.MEMBER_MEI_KANA AS MEMBER_KANA_NM,
			ISNULL(M.MEMBER_POST,'') AS MEMBER_POST_NO,
			ISNULL(M.MEMBER_ADD1+M.MEMBER_ADD2,'') AS MEMBER_ADD1,
			ISNULL(M.MEMBER_ADD3,'') AS MEMBER_ADD2,
			ISNULL(M.MEMBER_TEL,'') AS MEMBER_TEL_NO,
			ISNULL(M.MEMBER_MOBILE_TEL,'') AS MEMBER_MOBIL_TEL_NO,
			ISNULL(M.MEMBER_MAIL,'') AS MEMBER_MAIL,
			REPLACE(M.MEMBER_BIRTHDAY, '-','') AS MEMBER_BIRTH_DAY,
			M.MEMBER_SEX_KBN AS MEMBER_SEX_KBN,
			REPLACE(M.MEMBER_ENTRY_DATE,'-','') AS MEMBER_ENTRY_DATE,
			ISNULL(M.MEMBER_SYOUMEI_FLG,'') AS MEMBER_SYOUMEI_FLG,
			ISNULL(M.MEMBER_SYOUMEI_KBN,'') AS MEMBER_SYOUMEI_KBN,
			ISNULL(M.MEMBER_SYOUMEI_NO,'') AS MEMBER_SYOUMEI_NO,
			'0' AS JOB_CD,
			M.MEMBER_POINT AS NOW_POINT,
			M.LAST_LOGIN_DATE AS LAST_NYUTEN_DATE,
			M.LOGIN_CNT AS NYUTEN_CNT,
			M.BLACK_FLG,
			ISNULL(M.RPT_COM,'') AS  RPT_COM,
			ISNULL(C.MEMBER_SHOP_COM,'') AS SHOP_COM,
			'1' As MEMBER_FLG,
			'02' AS MEMBER_LEVEL,
			'0' AS AGE_CD,
			M.SMOKER_FLG,
			M.DELETE_FLG,
			ISNULL(M.OLD_MEMBER_ID,'') AS OLD_MEMBER_ID,
			ISNULL(M.MEMBER_SYOUMEI_COM,'') AS MEMBER_SYOUMEI_COM
			FROM MST_MEMBER M
			LEFT JOIN MST_MEMBER_SHOP C  ON M.MEMBER_ID = C.MEMBER_ID
			WHERE M.MEMBER_ID = @MEMBER_ID`;
			let MST_MEMBER = await pool.request()
			.input('SHOP_FC_NO', sql.VarChar, shopFcNo)
			.input('SHOP_NAME', sql.VarChar, shopName)
			.input('MEMBER_ID', sql.VarChar, memberID)
			.query(SQL);
			if(MST_MEMBER.recordset.length !== 0) {
				MST_MEMBER = MST_MEMBER.recordset[0];
				const xml = builder.create({
					MEMBER: {
						XML_MEMBER: {
							SHOP_FC_NO: MST_MEMBER.SHOP_FC_NO,
							SHOP_NM: MST_MEMBER.SHOP_NM,
							MEMBER_ID: MST_MEMBER.MEMBER_ID,
							MEMBER_NM: MST_MEMBER.MEMBER_NM,
							MEMBER_KANA_NM: MST_MEMBER.MEMBER_KANA_NM,
							MEMBER_POST_NO: MST_MEMBER.MEMBER_POST_NO,
							MEMBER_ADD1: MST_MEMBER.MEMBER_ADD1,
							MEMBER_ADD2: MST_MEMBER.MEMBER_ADD2,
							MEMBER_TEL_NO: MST_MEMBER.MEMBER_TEL_NO,
							MEMBER_MOBIL_TEL_NO: MST_MEMBER.MEMBER_MOBIL_TEL_NO,
							MEMBER_MAIL: MST_MEMBER.MEMBER_MAIL,
							MEMBER_BIRTH_DAY: MST_MEMBER.MEMBER_BIRTH_DAY,
							MEMBER_SEX_KBN: MST_MEMBER.MEMBER_SEX_KBN,
							MEMBER_ENTRY_DATE: MST_MEMBER.MEMBER_ENTRY_DATE,
							MEMBER_SYOUMEI_FLG: MST_MEMBER.MEMBER_SYOUMEI_FLG,
							MEMBER_SYOUMEI_KBN: MST_MEMBER.MEMBER_SYOUMEI_KBN,
							MEMBER_SYOUMEI_NO: MST_MEMBER.MEMBER_SYOUMEI_NO,
							JOB_CD: MST_MEMBER.JOB_CD,
							NOW_POINT: MST_MEMBER.NOW_POINT,
							LAST_NYUTEN_DATE: MST_MEMBER.LAST_NYUTEN_DATE,
							NYUTEN_CNT: MST_MEMBER.NYUTEN_CNT,
							BLACK_FLG: MST_MEMBER.BLACK_FLG,
							RPT_COM: MST_MEMBER.RPT_COM,
							SHOP_COM: MST_MEMBER.SHOP_COM,
							MEMBER_FLG: MST_MEMBER.MEMBER_FLG,
							MEMBER_LEVEL: MST_MEMBER.MEMBER_LEVEL,
							AGE_CD: MST_MEMBER.AGE_CD,
							SMOKER_FLG: MST_MEMBER.SMOKER_FLG,
							DELETE_FLG: MST_MEMBER.DELETE_FLG,
							OLD_MEMBER_ID: MST_MEMBER.OLD_MEMBER_ID,
							MEMBER_SYOUMEI_COM: MST_MEMBER.MEMBER_SYOUMEI_COM
						}
					}
				})
				.end();
				let url = `http://210.233.78.100/reg.php?shop_fc_no=${MST_MEMBER.SHOP_FC_NO}`;
				let _POST_XML_MEMBER = await POST_XML_MEMBER(url,xml)
				.then(async(result) => {
					if(!result) {
						SQL = "SELECT MAX(ID) AS ID FROM TBL_ERR_XML"
						let idNum = await pool.request()
						.query(SQL);
						idNum = (idNum.recordset[0].ID != null)?idNum.recordset[0].ID + 1:0;

						SQL = "INSERT INTO TBL_ERR_XML (ID, XML, INPUT_DATE, INPUT_STAFF_ID) VALUES (@ID, @XML, GETDATE(), @INPUT_STAFF_ID)";
						let INSERT_ERR_XML = await pool.request()
						.input('ID', sql.Int, idNum)
						.input('XML', sql.VarChar, xml)
						.input('INPUT_STAFF_ID', sql.VarChar, CNST_STAFF_ID)
						.query(SQL);
						if(INSERT_ERR_XML.rowsAffected[0] == 1) {
							REQUEST_LOG(req,`INFO: LOG_XML inserted`);
							success = true;
						}
					} else {
				...
				.catch((err) => {
					REQUEST_LOG(req,`ERROR: POST_XML_MEMBER ${err}`);
					sql.close();
					return res.status(200).send(CNST_ERROR_CODE.error_11);
				});
			...
			REQUEST_LOG(req,`ERROR: _ICPOS_XML_MEMBER ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
		}
		return success;
	...
	async function POST_XML_MEMBER(url,xml) {
		const request = require('request');
		let success = false;
		...
		return new Promise((resolve,reject) => {
			request(request_opt, (err, httpResponse, body) => {
				if(err) {
					REQUEST_LOG(req,`X-SITE: error request ${xml}`);
					sql.close();
					return res.status(200).send(error);
				}
				REQUEST_LOG(req,`X-SITE: ${body} ${xml}`);
				if(body == 1) {
					success = true;
					resolve(success);
				} else {
	...
	async function UPLOAD_MST_MEMBER(memberID,shopFcNo) {
		...
		try {
			SQL = "SELECT @SHOP_FC_NO AS SHOP_FC_NO, * FROM MST_MEMBER WHERE MEMBER_ID = @MEMBER_ID";
			let MST_MEMBER = await pool.request()
			.input('SHOP_FC_NO', sql.VarChar, shopFcNo)
			.input('MEMBER_ID', sql.VarChar, memberID)
			.query(SQL);
			if(MST_MEMBER.recordset.length !== 0) {
				UploadFlag = (MST_MEMBER.recordset[0].UPLOAD_FLG === 0)?false:true;
				if(UploadFlag) {
					MST_MEMBER.recordset[0].CARD_UPDATE_DATE = (MST_MEMBER.recordset[0].CARD_UPDATE_DATE != null)?convert_datetime(MST_MEMBER.recordset[0].CARD_UPDATE_DATE):null;
					MST_MEMBER.recordset[0].INPUT_DATE = (MST_MEMBER.recordset[0].INPUT_DATE != null)?convert_datetime(MST_MEMBER.recordset[0].INPUT_DATE):null;
					MST_MEMBER.recordset[0].LAST_LOGIN_DATE = (MST_MEMBER.recordset[0].LAST_LOGIN_DATE != null)?convert_datetime(MST_MEMBER.recordset[0].LAST_LOGIN_DATE):null;
					MST_MEMBER.recordset[0].MEMBER_BIRTHDAY = (MST_MEMBER.recordset[0].MEMBER_BIRTHDAY != null)?convert_datetime(MST_MEMBER.recordset[0].MEMBER_BIRTHDAY):null;
					MST_MEMBER.recordset[0].MEMBER_ENTRY_DATE = (MST_MEMBER.recordset[0].MEMBER_ENTRY_DATE != null)?convert_datetime(MST_MEMBER.recordset[0].MEMBER_ENTRY_DATE):null;
					MST_MEMBER.recordset[0].UPDATE_DATE = (MST_MEMBER.recordset[0].UPDATE_DATE != null)?convert_datetime(MST_MEMBER.recordset[0].UPDATE_DATE):null;

					let _MST_MEMBER_ = MST_MEMBER.recordset[0];

					let MST_MEMBER_ = {
						"APP_LOGIN_CNT":_MST_MEMBER_.APP_LOGIN_CNT,
						"APP_MEMBER_ID":_MST_MEMBER_.APP_MEMBER_ID,
						"BLACK_FLG":_MST_MEMBER_.BLACK_FLG,
						"CARD_CHANGE_CNT":_MST_MEMBER_.CARD_CHANGE_CNT,
						"CARD_FLG":_MST_MEMBER_.CARD_FLG,
						"CARD_UPDATE_DATE":_MST_MEMBER_.CARD_UPDATE_DATE,
						"DELETE_FLG":_MST_MEMBER_.DELETE_FLG,
						"INPUT_DATE":_MST_MEMBER_.INPUT_DATE,
						"INPUT_STAFF_ID":_MST_MEMBER_.INPUT_STAFF_ID,
						"LAST_LOGIN_DATE":_MST_MEMBER_.LAST_LOGIN_DATE,
						"LOGIN_CNT":_MST_MEMBER_.LOGIN_CNT,
						"MEMBER_ADD1":_MST_MEMBER_.MEMBER_ADD1,
						"MEMBER_ADD2":_MST_MEMBER_.MEMBER_ADD2,
						"MEMBER_ADD3":_MST_MEMBER_.MEMBER_ADD3,
						"MEMBER_BIRTHDAY":_MST_MEMBER_.MEMBER_BIRTHDAY,
						"MEMBER_ENTRY_DATE":_MST_MEMBER_.MEMBER_ENTRY_DATE,
						"MEMBER_ID":_MST_MEMBER_.MEMBER_ID,
						"MEMBER_LEVEL":_MST_MEMBER_.MEMBER_LEVEL,
						"MEMBER_MAIL":_MST_MEMBER_.MEMBER_MAIL,
						"MEMBER_MEI":_MST_MEMBER_.MEMBER_MEI,
						"MEMBER_MEI_KANA":_MST_MEMBER_.MEMBER_MEI_KANA,
						"MEMBER_MOBILE_TEL":_MST_MEMBER_.MEMBER_MOBILE_TEL,
						"MEMBER_NM":_MST_MEMBER_.MEMBER_NM,
						"MEMBER_POINT":_MST_MEMBER_.MEMBER_POINT,
						"MEMBER_POST":_MST_MEMBER_.MEMBER_POST,
						"MEMBER_SEI":_MST_MEMBER_.MEMBER_SEI,
						"MEMBER_SEI_KANA":_MST_MEMBER_.MEMBER_SEI_KANA,
						"MEMBER_SEX_KBN":_MST_MEMBER_.MEMBER_SEX_KBN,
						"MEMBER_SYOUMEI_COM":_MST_MEMBER_.MEMBER_SYOUMEI_COM,
						"MEMBER_SYOUMEI_FLG":_MST_MEMBER_.MEMBER_SYOUMEI_FLG,
						"MEMBER_SYOUMEI_KBN":_MST_MEMBER_.MEMBER_SYOUMEI_KBN,
						"MEMBER_SYOUMEI_NO":_MST_MEMBER_.MEMBER_SYOUMEI_NO,
						"MEMBER_TEL":_MST_MEMBER_.MEMBER_TEL,
						"NEXT_MEMBER_ID":_MST_MEMBER_.NEXT_MEMBER_ID,
						"OLD_MEMBER_ID":_MST_MEMBER_.OLD_MEMBER_ID,
						"RPT_COM":_MST_MEMBER_.RPT_COM,
						"SHOP_FC_NO":_MST_MEMBER_.SHOP_FC_NO,
						"SMOKER_FLG":_MST_MEMBER_.SMOKER_FLG,
						"SYOUMEI_SHOP_FC_NO":_MST_MEMBER_.SYOUMEI_SHOP_FC_NO,
						"SYOUMEI_STAFF_NM":_MST_MEMBER_.SYOUMEI_STAFF_NM,
						"TEMP_DEL_FLG":_MST_MEMBER_.TEMP_DEL_FLG,
						"UPDATE_DATE":_MST_MEMBER_.UPDATE_DATE,
						"UPDATE_SHOP_FC":_MST_MEMBER_.UPDATE_SHOP_FC,
						"UPDATE_STAFF_ID":_MST_MEMBER_.UPDATE_STAFF_ID,
						"UPLOAD_FLG":_MST_MEMBER_.UPLOAD_FLG
					};

					uploadJson.json.push(MST_MEMBER_);
					...
					const request_opt = {
						...
						body: uploadJson,
						json: true,
						// url: 'http://acrossweb.net/upload/member'
						url: 'http://api.across-web.net/upload/member'
					};

					request(request_opt,(err, httpResponse, body) => {
						if(err) {
							REQUEST_LOG(req,`X-SITE: error request ${JSON.stringify(uploadJson)}`);
							sql.close();
			...
			REQUEST_LOG(req,`ERROR: UPLOAD_MST_MEMBER${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
		}
		return success;
	...
	async function UPDATE_TBL_GATE(uriage,allTotalYen,poolPrice,SEQ = 0) {
		let SQL = "";
		const SEISAN_DATE = dateTimeNow();
		try{
			...
			let flg = () => {
				return {
					OPEN_FLG: (allTotalYen >= poolPrice)?1:0,
					PRICE_LIMIT_FLG: (allTotalYen >= poolPrice)?1:0
				};
			};
			...
			transaction.begin(err => {
				...
				transaction.on('rollback',async aborted => {
					REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_5} UPDATE_TBL_GATE rollback ${err}`);
					sql.close();
					return res.status(200).json(CNST_ERROR_CODE.error_5);
				});
				...
				SQL = "UPDATE TBL_GATE SET OPEN_FLG = @OPEN_FLG , LOGIN_FLG = @LOGIN_FLG, PRICE_LIMIT_FLG = @PRICE_LIMIT_FLG, SEISAN_DATE = @SEISAN_DATE WHERE SEAT_NO = @SEAT_NO AND SEQ = (SELECT MIN(SEQ) AS SEQ FROM TBL_GATE WHERE SEAT_NO = @SEAT_NO AND LOGIN_FLG = 1 AND OPEN_FLG = 1 AND SEISAN_DATE IS NULL) AND SEISAN_DATE IS NULL";
				transaction.request()
				.input("SEAT_NO",sql.VarChar,uriage.SEAT_NO)
				.input("SEISAN_DATE",sql.VarChar,uriage.SEISAN_DATE)
			...
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} UPDATE_TBL_GATE ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
		}
	}
	...
	async function MULTIPLE_LOGIN(seatNo) {
		...
		try {
			...
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} MULTIPLE_LOGIN ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
	...
	async function GET_MAX_SALES_SEQ(salesNo) {
		...
		try {
			...
			if(result.recordset.length > 0) {
				...
				REQUEST_LOG(req,`WARN: ${CNST_ERROR_CODE.error_11} GETMAX_SALES_SEQ no match found`);
				sql.close();
				return res.status(200).send(CNST_ERROR_CODE.error_11);
			...
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} GET_MAX_SALES_SEQ ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
	...
	async function TBL_URIAGE(SEAT_NO,SALES_NO) {
		try{
			...
			if(result.recordset.length > 0) {
				...
				REQUEST_LOG(req,`WARN: ${CNST_ERROR_CODE.error_2} TBL_URIAGE no data found`);
				sql.close();
				return res.status(200).send(CNST_ERROR_CODE.error_2);
			}
		} catch(err) {
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} TBL_URIAGE ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
		}
	...
	async function MST_SHOP() {
		try {
			...
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} MST_SHOP ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_11);
			// ERROR_LOGGER(0,'MST_SHOP: '+err);
		}
...
app.post('/api/coupon_search', async (req,res) => {
	...
	try{
		...
		.then(async result => {
			...
			let coupon_rules = {
				"member_id":"Required pos_members",
			...
			.then(async result => {
				...
				REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_3} 2nd level ${err}`);
				return res.status(200).send(CNST_ERROR_CODE.error_3);
			});

		})
		.catch(err => {
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_3} 1st level ${err}`);
			return res.status(200).send(CNST_ERROR_CODE.error_3);
		});

	} catch(err) {
		REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_1} API-COUPON_SEARCH ${err}`);
		return res.status(200).send(CNST_ERROR_CODE.error_1);
	}
		...
		try {
			...
			request(request_opt,(err, httpResponse, body) => {
				...
				if(errorCode[body.code]() !== 'undefined') {
					REQUEST_LOG(req,`XSITE: ${errorCode[body.code]()}`);
					return res.status(200).send(errorCode[body.code]());
				} else {
					REQUEST_LOG(req,`XSITE: ${errorCode['default']()}`);
					return res.status(200).send(errorCode['default']());
				}

			});

		} catch(err) {
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} sendRequestAPI ${err}`);
			return res.status(200).send(CNST_ERROR_CODE.error_11);
		}
	}
...
//#region API-COUPON_CANCEL
app.post('/api/coupon_cancel', async (req,res) => {
	...
	function sendRequestAPI(coupon_search) {
		...
		try {
			...
			const request_opt = {
				...
				body: postData,
				json: true,
				url: 'http://coupon.across-web.net/api/v2/machine/cancel/coupon'
			};
			...
			request(request_opt,(err, httpResponse, body) => {
				...
				let errorCode = {
					...
					3: () => { return CNST_ERROR_CODE.error_2; },
					4: () => { return CNST_ERROR_CODE.error_4; },
					1: () => { return CNST_ERROR_CODE.error_0; },
					6: () => { return CNST_ERROR_CODE.error_2; },
					'default': () => { return CNST_ERROR_CODE.error_11; }
				};

				if(errorCode[code]() !== 'undefined') {
					REQUEST_LOG(req,`XSITE: ${errorCode[code]()} ${JSON.stringify(body)}`);
					REQUEST_LOG(req,`RESPONSE: ${code}`);
					return res.status(200).send(errorCode[code]());
				} else {
					REQUEST_LOG(req,`XSITE: ${errorCode['default']()} ${JSON.stringify(body)}`);
					REQUEST_LOG(req,`RESPONSE: ${code}`);
					return res.status(200).send(errorCode['default']());
				}
...
app.post('/api/cancel', async (req,res) => {
	...
	try {
		...
		.then(async result => {
			...
			transaction.begin(async err => {
				...
				transaction.on('rollback', aborted => {
					REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} API-CANCEL rolledback`);
					sql.close();
					return res.status(200).send(CNST_ERROR_CODE.error_11);
				});
				transaction.on('commit', () => {
					REQUEST_LOG(req,`RESPONSE: ${CNST_ERROR_CODE.error_0} API-CANCEL committed`);
					sql.close();
					return res.status(200).send(CNST_ERROR_CODE.error_0);
				});
				...
				.query(SQL,async(err,result) => {
					...
					if(result.rowsAffected[0] !== 0) {
						...
						REQUEST_LOG(req,`WARN: ${CNST_ERROR_CODE.error_2} API-CANCEL Update failed`);
						sql.close();
						return res.status(200).send(CNST_ERROR_CODE.error_2);
					}
				});
		...
		.catch(err => {
			REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_2} API-CANCEL Validation error ${err}`);
			sql.close();
			return res.status(200).send(CNST_ERROR_CODE.error_2);
		});

	} catch(err) {
		REQUEST_LOG(req,`ERROR: ${CNST_ERROR_CODE.error_11} API-CANCEL ${err}`);
		sql.close();
		return res.status(200).send(CNST_ERROR_CODE.error_11);
	}
...
app.post('/testme', async (req,res) => {
	...
	// let _ICPOS_XML_MEMBER = await ICPOS_XML_MEMBER('999910000002','99997', 'TEST_SHOP');
	// if(_ICPOS_XML_MEMBER) {
	// 	REQUEST_LOG(req,`RESPONSE: ${_ICPOS_XML_MEMBER}`);
	// 	sql.close();
	// 	return res.status(200).send(CNST_ERROR_CODE.error_0);
	// } else {
	// 	REQUEST_LOG(req,`RESPONSE: ${_ICPOS_XML_MEMBER}`);
	...
	function POST_XML_MEMBER(url,xml) {
		const request = require('request');
		let success = false;
